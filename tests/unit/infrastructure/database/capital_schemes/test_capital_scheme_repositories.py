...
class TestDatabaseCapitalSchemeRepository:
        ...
        async with AsyncSession(engine) as session, session.begin():
            session.add_all(
                [
                    ...
                    entities.build_capital_scheme(reference="ATE00002"),
                ]
            )
    ...
    async def test_get_items_by_bid_submitting_authority(self, engine: AsyncEngine, entities: EntityBuilder) -> None:
        async with AsyncSession(engine) as session, session.begin():
            session.add_all(
                [
                    liv := build_authority_entity(abbreviation="LIV"),
                    ...
                    entities.build_capital_scheme(
                        reference="ATE00003",
                        overviews=[entities.build_capital_scheme_overview(bid_submitting_authority=wyo)],
                    ),
                ]
            )
    ...
    async def test_get_items_by_bid_submitting_authority_fetches_current_overview(
        self, engine: AsyncEngine, entities: EntityBuilder
    ) -> None:
        async with AsyncSession(engine) as session, session.begin():
            session.add_all(
                [
                    ...
                    entities.build_capital_scheme(
                        ...
                        overviews=[
                            ...
                            entities.build_capital_scheme_overview(
                                bid_submitting_authority=wyo, effective_date_from=local_datetime(2020, 2, 1)
                            ),
                        ],
                    ),
                ]
            ...
            session.add_all(
                [
                    ...
                    entities.build_capital_scheme(
                        reference="ATE00001",
                        overviews=[
                            entities.build_capital_scheme_overview(bid_submitting_authority=liv, funding_programme=atf3)
                        ],
                    ),
                    entities.build_capital_scheme(
                        reference="ATE00002",
                        overviews=[
                            entities.build_capital_scheme_overview(bid_submitting_authority=liv, funding_programme=atf4)
                        ],
                    ),
                    entities.build_capital_scheme(
                        reference="ATE00003",
                        overviews=[
                            entities.build_capital_scheme_overview(bid_submitting_authority=liv, funding_programme=atf5)
                        ],
                    ),
                ]
            )
    ...
    async def test_get_items_by_bid_submitting_authority_when_no_overview(
        self, engine: AsyncEngine, entities: EntityBuilder
    ) -> None:
        async with AsyncSession(engine) as session, session.begin():
            session.add_all(
    ...
    async def test_get_items_by_bid_submitting_authority_when_no_status(
        self, engine: AsyncEngine, entities: EntityBuilder
    ) -> None:
        async with AsyncSession(engine) as session, session.begin():
            session.add_all(
        ...
        async with AsyncSession(engine) as session, session.begin():
            session.add_all(
                [
                    ...
                    imp2 := entities.build_improvement(
                        reference="IMP00002",
                    ...
                    entities.build_capital_scheme(
                        reference="ATE00003",
                        overviews=[entities.build_capital_scheme_overview(improvement=imp2)],
                    ),
                ]
            )
                ...
                [
                    ...
                    entities.build_capital_scheme(
                        ...
                        overviews=[
                            ...
                            entities.build_capital_scheme_overview(
                                improvement=imp2, effective_date_from=local_datetime(2020, 2, 1)
                            ),
                        ],
                    ),
                ]
