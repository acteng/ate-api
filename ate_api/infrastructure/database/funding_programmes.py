...
class DatabaseFundingProgrammeRepository(FundingProgrammeRepository):
    ...
    async def get(self, code: FundingProgrammeCode) -> FundingProgramme | None:
        result = await self._session.scalars(
            select(FundingProgrammeEntity).where(FundingProgrammeEntity.funding_programme_code == str(code))
        )
        row = result.one_or_none()
        return row.to_domain() if row else None

    async def get_all(self, is_eligible_for_authority_update: bool | None = None) -> list[FundingProgramme]:
        statement = select(FundingProgrammeEntity).order_by(FundingProgrammeEntity.funding_programme_code)
    ...
    async def exists(self, code: FundingProgrammeCode) -> bool:
        result = await self._session.scalars(
            select(exists().where(FundingProgrammeEntity.funding_programme_code == str(code)))
        )
        return result.one()

    async def exists_all(self, codes: list[FundingProgrammeCode]) -> bool:
        result = await self._session.scalars(
            select(count()).where(FundingProgrammeEntity.funding_programme_code.in_(str(code) for code in codes))
        )
